...
And then include `phmap.h` as follows:
```cxx
// main.cxx
#include "node_modules/phmap.cxx/phmap.h"
...
If you add the path `node_modules/phmap.cxx` to your compiler's include paths.

```bash
$ clang++ -I./node_modules/phmap.cxx main.cxx  # or, use g++
$ g++     -I./node_modules/phmap.cxx main.cxx
```
...
The header `phmap/phmap.h` provides the implementation for the following eight hash tables:
- phmap::flat_hash_set
- phmap::flat_hash_map
- phmap::node_hash_set
- phmap::node_hash_map
...
The header `phmap/btree.h` provides the implementation for the following btree-based ordered containers:
- phmap::btree_set
- phmap::btree_map
...
The full types with template parameters can be found in the [phmap/phmap_fwd_decl.h](https://raw.githubusercontent.com/greg7mdp/parallel-hashmap/master/phmap/phmap_fwd_decl.h) header, which is useful for forward declaring the Parallel Hashmaps when necessary.
...
```c++
#include <phmap/phmap_utils.h> // minimal header providing phmap::HashState()
#include <string>
using std::string;
...
![](https://ga-beacon.deno.dev/G-RC63DPBH3P:SH3Eq-NoQ9mwgYeHWxu7cw/github.com/nodef/phmap.cxx)
